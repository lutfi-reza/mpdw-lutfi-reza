...
summary(model)
```
$$y_i = -2.195*10^4+1.122*10^1+3.139*10^2-3.525*10^3$$
...
dwtest(modelCO)
```
Model menggunakan Cochrane-Orcutt $$y_i=-37714.925547+19.166264x_t-0.009780 x_(t-1)-0.000401x_(t-2)$$.

Nilai DW setelah dilakukan penanganan adalah $DW = 1.4258$. Hasil ini menunjukkan nilai DW sudah berada dalam selang antara DL dan DU sehingga sudah tidak ada autokorelasi positif. 
...
dwtest(modelHL)
```
Hasil uji Durbin-Watson juga menunjukkan bahwa nilai DW sebesar $1.426$
berada pada selang daerah tidak ada autokorelasi, yaitu pada rentang DL
\< DW \< DU. 
...
akurasi
```
Berdasarkan hasil tersebut dapat diketahui bahwa hasil penanganan autokorelasi dengan metode Cochrane-Orcutt dan Hildreth-Lu memiliki SSE yang sama, sebesar $125480.836$ dan lebih baik dibandingkan model awal ketika autokorelasi masih terjadi, yaitu sebesar $403335.477$.
